...
4. **Intent classification** — Zero-shot classification via `facebook/bart-large-mnli`
...
nlu/                     # Natural Language Understanding pipeline
  preprocessor.py        # Stop-word removal & lemmatization
  intent_classifier.py   # Zero-shot classification (BART-MNLI)
...
Type natural-language commands at the prompt. The BART model warms up on first launch (may take a few seconds).
...
| NLU model | `facebook/bart-large-mnli` | Zero-shot classifier model |
...
| Library | Purpose |
|---|---|
| [transformers](https://huggingface.co/docs/transformers) | Zero-shot classification & summarization |
| [torch](https://pytorch.org/) | PyTorch backend for transformer models |
...
- **Phase 2** ✅ Text-mode interactive assistant (current)
